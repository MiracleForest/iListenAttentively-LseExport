...
ValueType pack(T val) {
    ...
    } else if constexpr (ll::concepts::IsExpected<RawType>) {
        ValueType errorField{Value{std::nullptr_t{}}};
        ValueType valueField{std::nullptr_t{}};

        if (val.has_value()) {
            if constexpr (!std::is_void_v<typename RawType::value_type>) {
                valueField = pack(std::move(val.value()));
            }
        } else {
            errorField = val.error().message();
        }

        return std::unordered_map<std::string, RemoteCall::ValueType>{
            {"success", val.has_value()      },
            {"value",   std::move(valueField)},
            {"error",   std::move(errorField)}
        };
    } else if constexpr (std::is_same_v<RawType, ValueType>) {
        return std::move(val);
...
inline bool
_exportAs(std::string const& funcName, std::function<RTN(Args...)>&& callback) {
    CallbackFn cb = [callback = std::move(callback)](std::vector<ValueType> args) -> ValueType {
        int index = sizeof...(Args);
        try {
            if constexpr (ll::concepts::IsExpected<RTN>) {
                if constexpr (requires { callback(args); }) {
                    return pack(callback(args));
                } else {
                    if (sizeof...(Args) != args.size()) return pack(RTN(ll::makeStringError("Invalid argument")));
            ...
            if constexpr (ll::concepts::IsExpected<RTN>) {
                return pack(RTN(ll::makeExceptionError()));
            } else {
                return pack(ll::Expected<RTN>(ll::makeExceptionError()));
            }
        }
    };
    return exportFunc("iListenAttentively", funcName, std::move(cb), ll::sys_utils::getCurrentModuleHandle());
}
...
inline bool exportAs(std::string const& funcName, CB&& callback) {
    return _exportAs(funcName, std::function(std::forward<CB>(callback)));
}
