...
#define EXPORT_IMPL(VERSION)                                                                                             \
    ...
    namespace ila {                                                                                                      \
    void LseExport::exportEvent##VERSION() {                                                                             \
        ...
        RemoteCall::exportAs(                                                                                            \
            "toSnbt",                                                                                                    \
            [&](CompoundTag* nbt, uint snbtFormat, uchar indent) -> ll::Expected<std::string> {                          \
        ...
        EXPORT_INTEGER64_GETTER_SETTER("LongLong", int64);                                                               \
        EXPORT_INTEGER64_GETTER_SETTER("UnsignedLongLong", uint64);                                                      \
        ...
        RemoteCall::exportAs("ctorString", [&](std::vector<RemoteCall::ValueType> args) -> ll::Expected<> {              \
            if (args.size() < 1 || args.size() > 2) return ll::makeStringError("Too many arguments");                    \
            auto info = RemoteCall::extract<uintptr_t>(std::move(args[0]));                                              \
            if (args.size() >= 2) {                                                                                      \
                auto value = RemoteCall::extract<std::string>(std::move(args[1]));                                       \
                ...
                reinterpret_cast<std::string*>(info)->std::string::basic_string(value);                                  \
            } else {                                                                                                     \
                reinterpret_cast<std::string*>(info)->std::string::basic_string();                                       \
            }                                                                                                            \
            return {};                                                                                                   \
        });                                                                                                              \
        ...
        RemoteCall::exportAs(                                                                                            \
            "getStringView",                                                                                             \
            [&](std::vector<RemoteCall::ValueType> args) -> ll::Expected<std::string> {                                  \
                if (args.size() < 1 || args.size() > 3) return ll::makeStringError("Too many arguments");                \
                auto        info        = RemoteCall::extract<uintptr_t>(std::move(args[0]));                            \
                bool        base64      = args.size() >= 2 ? RemoteCall::extract<bool>(std::move(args[1])) : false;      \
                bool        pauseThread = args.size() >= 3 ? RemoteCall::extract<bool>(std::move(args[2])) : false;      \
        ...
        RemoteCall::exportAs("getUsableMemorySize", [&](uintptr_t address) {                                             \
            ...
            if (ll::getGameVersion() >= ll::data::Version{1, 21, 70}) {                                                  \
                return ll::memory::virtualCall<uint64, void*, bool>(                                                     \
                    &allocator,                                                                                          \
                    5,                                                                                                   \
        ...
        RemoteCall::exportAs(                                                                                            \
            ...
            [this](std::vector<RemoteCall::ValueType> args) -> ll::Expected<HookManager::HookId> {                       \
                ...
                auto resultSig =                                                                                         \
                    resultType == NativeTypes::LongDouble ? DC_SIGCHAR_DOUBLE : static_cast<DCsigchar>(resultType);      \
                ...
                static constexpr auto readArguments =                                                                    \
                    [](DCArgs* nativeArgs, std::vector<NativeTypes> const& paramsTypes, bool asString) {                 \
                        ...
                        return params;                                                                                   \
                                                                                                                         ...
                                                                                                                         \
                static constexpr auto writeResult = [](DCValue*              nativeResult,                               \
                                                       NativeTypes           resultType,                                 \
        ...
        RemoteCall::exportAs(                                                                                            \
            ...
            [&](std::vector<RemoteCall::ValueType> args) -> ll::Expected<RemoteCall::ValueType> {                        \
                ...
                if (!magic_enum::enum_contains<NativeTypes>(resultType))                                                 \
                    return ll::makeStringError("Invalid result type");                                                   \
                auto paramsTypes = args.size() > 2 ? RemoteCall::extract<std::vector<NativeTypes>>(std::move(args[2]))   \
                                                   : std::vector<NativeTypes>{};                                         \
                auto params      = args.size() > 3                                                                       \
                                     ? RemoteCall::extract<std::vector<RemoteCall::ValueType>>(std::move(args[3]))       \
                                     : std::vector<RemoteCall::ValueType>{};                                             \
                ...
                for (size_t i = 0; i < params.size(); ++i) {                                                             \
                    ...
                    case NativeTypes::UnsignedChar:                                                                      \
                        dcArgChar(                                                                                       \
                            dynamicCallVM,                                                                               \
                            std::bit_cast<char>(RemoteCall::extract<uchar>(std::move(params[i])))                        \
                        );                                                                                               \
                        break;                                                                                           \
                    case NativeTypes::Short:                                                                             \
                    ...
                    case NativeTypes::UnsignedLong:                                                                      \
                        dcArgLong(                                                                                       \
                            dynamicCallVM,                                                                               \
                            std::bit_cast<long>(RemoteCall::extract<ulong>(std::move(params[i])))                        \
                        );                                                                                               \
                        break;                                                                                           \
                    case NativeTypes::LongLong:                                                                          \
                ...
                default:                                                                                                 \
                    return ll::makeStringError("Unknown result type");                                                   \
                                                                                                                         ...
                                                                                                                         \
    void LseExport::registerDefaultEventsAlias##VERSION() {                                                              \
        ...
        for (auto event : ll::event::EventBus::getInstance().events("LeviLamina")) {                                     \
            ...
            mEventNameAlias[fmt::format("ll::event::{0}", event.name.substr(event.name.rfind("::") + 2))] =              \
                event.name;                                                                                              \
        }                                                                                                                \
    }                                                                                                                    \
    }
