const iListenAttentively = {
    ...
    getPlayer(address) {
        ...
        const result = ll.imports("iListenAttentively", "getPlayer")(address);
        return result?.uuid !== undefined ? result : undefined;
    },
    getEntity(address) {
        if (address?.get) address = address.get();
        if (address instanceof NbtCompound) address = this.nbtProxyHandler.get(address, "_pointer_");
        const result = ll.imports("iListenAttentively", "getEntity")(address);
        return result?.uniqueId !== undefined ? result : undefined;
    },
    getItem(address) {
        if (address?.get) address = address.get();
        if (address instanceof NbtCompound) address = this.nbtProxyHandler.get(address, "_pointer_");
        const result = ll.imports("iListenAttentively", "getItem")(address);
        return result?.type !== undefined ? result : undefined;
    },
    getBlock(address) {
        if (address?.get) address = address.get();
        if (address instanceof NbtCompound) address = this.nbtProxyHandler.get(address, "_pointer_");
        const result = ll.imports("iListenAttentively", "getBlock")(address);
        return result?.type !== undefined ? result : undefined;
    },
    getBlockEntity(address) {
        if (address?.get) address = address.get();
        if (address instanceof NbtCompound) address = this.nbtProxyHandler.get(address, "_pointer_");
        const result = ll.imports("iListenAttentively", "getBlockEntity")(address);
        return result?.type !== undefined ? result : undefined;
    },
    getContainer(address) {
        if (address?.get) address = address.get();
        if (address instanceof NbtCompound) address = this.nbtProxyHandler.get(address, "_pointer_");
        const result = ll.imports("iListenAttentively", "getContainer")(address);
        return result?.type !== undefined ? result : undefined;
    }
};
...
NbtList.prototype.toSNBT = function (index = -1) {
    ...
    ['{', 'd', 'a', 't', 'a', ':'].forEach(str => result = result.slice(result.indexOf(str) + 1));
