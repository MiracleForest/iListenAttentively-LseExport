const iListenAttentively = {
    ...
    getItem(info) {
        if (info?.get) info = info.get();
        const result = ll.imports("iListenAttentively", "getItem")(info);
        return result?.type !== undefined ? result : undefined;
    },
    getBlock(info) {
        if (info?.get) info = info.get();
        const result = ll.imports("iListenAttentively", "getBlock")(info);
        return result?.type !== undefined ? result : undefined;
    },
    getBlockEntity(info) {
        if (info?.get) info = info.get();
        const result = ll.imports("iListenAttentively", "getBlockEntity")(info);
        return result?.type !== undefined ? result : undefined;
    },
    getContainer(info) {
        if (info?.get) info = info.get();
        const result = ll.imports("iListenAttentively", "getContainer")(info);
        return result?.type !== undefined ? result : undefined;
    }
};
