...
module.exports = Object.assign(
    {
        ...
        NativeType: Object.freeze({
            ...
            Bool: "B".charCodeAt(0),
            Char: "c".charCodeAt(0),
            UnsignedChar: "C".charCodeAt(0),
            Short: "s".charCodeAt(0),
            UnsignedShort: "S".charCodeAt(0),
            Int: "i".charCodeAt(0),
            UnsignedInt: "I".charCodeAt(0),
            Long: "j".charCodeAt(0),
            UnsignedLong: "J".charCodeAt(0),
            LongLong: "l".charCodeAt(0),
            UnsignedLongLong: "L".charCodeAt(0),
            Float: "f".charCodeAt(0),
            Double: "d".charCodeAt(0),
            LongDouble: "D".charCodeAt(0),
            Pointer: "p".charCodeAt(0)
        }),
        ...
        NbtProxy: {
            FunctionMap: {
                        ...
                        switch (target.getType()) {
                            case NBT.List: {
                                for (let index = 0; index < target.getSize(); index++) {
                                    const value = proxy.get(target)(index);
                                    yield {
                                        index, i: index, key: index, k: index, first: index,
                                        value, v: value, second: value,
                            ...
                            case NBT.Compound: {
                                for (let key of target.getKeys()) {
                                    const value = proxy.get(target)(key);
                                    yield {
                                        index: key, i: key, key, k: key, first: key,
                                        value, v: value, second: value,
                                        [Symbol.iterator]: function* () {
                                            yield key;
                                            yield value;
                ...
                toPlayer(target) { return () => module.exports.getPlayer(target); },
                toActor(target) { return () => module.exports.getActor(target); },
                toItemStack(target) { return () => module.exports.getItemStack(target); },
                toBlock(target) { return () => module.exports.getBlock(target); },
                toBlockActor(target) { return () => module.exports.getBlockActor(target); },
                toContainer(target) { return () => module.exports.getContainer(target); },
                toCompoundTag(target) { return () => module.exports.getCompoundTag(target); }
            },
            get(target, key) {
            ...
            has(target, key) {
                return this.FunctionMap.contains(target)(key);
            },
            ownKeys(target) {
                switch (target.getType()) {
                    case NBT.List: return Array.from({ length: target.getSize() }, (_, i) => i.toString());
                    case NBT.Compound: return target.getKeys();
                    default: throw new Error(`Unsupported type ${target.getType()}`);
                }
            },
            getOwnPropertyDescriptor(target, key) {
                return this.has(target, key)
                    ? {
                        writable: true,
                        enumerable: true,
                        configurable: true
            ...
            getPrototypeOf(target) {
                const proto = { ...Reflect.getPrototypeOf(target) }; // copy prototype
                if (proto === null) return null;
                Reflect.ownKeys(proto).forEach(key => Object.defineProperty(proto, key, { enumerable: false }));
                return proto;
            }
        },
        isNbt(data) {
        ...
        callExportedFunction(functionName, originalReturn = false, ...args) {
            if (!ll.listPlugins().find(p => p === "iListenAttentively-LseExport")) throw new Error("iListenAttentively-LseExport is not loaded!");
            if (!ll.hasExported(module.exports.NamespaceName, functionName)) throw new Error(`Function ${module.exports.NamespaceName}.${functionName} does not exist`);
            const result = (module.exports[`${functionName}#Origin`] ??= ll.imports(module.exports.NamespaceName, functionName))(...args);
            if (originalReturn) return result;
            if (!result) throw new Error(`Function ${module.exports.NamespaceName}.${functionName} returned null`);
        ...
        getPluginName() {
            if (module.exports.PluginNameCache) return module.exports.PluginNameCache;

            return module.exports.PluginNameCache = (() => {
        ...
        registerEvent(eventName, pluginName = module.exports.getPluginName()) {
            return module.exports.callExportedFunction("registerEvent", false, pluginName, eventName);
        },
        RegisterEvent(...args) {
        ...
        emplaceListener(eventName, callback, priority = module.exports.EventPriority.Normal, pluginName = module.exports.getPluginName()) {
            ...
            const listenerId = module.exports.callExportedFunction("emplaceListener", false, pluginName, eventName, priority);
            ll.exports(event => {
                try {
        ...
        unhook(hookId, suspendThreads = true) {
            return module.exports.callExportedFunction("unhook", false, hookId, suspendThreads);
        },
        publish(...args) {
            ...
            if (!module.exports.isNbt(args[1])) {
                ...
                module.exports.callExportedFunction("publish", false, args[0], eventData, ...args.slice(2));
                Object.entries(module.exports.fromNbt(eventData)).forEach(([key, value]) => (args[1][key] = value));
            } else {
                module.exports.callExportedFunction("publish", false, ...args);
            }
        },
    },
    ...(() => {
        const normalizeInteger64 = value => typeof value === "bigint" ? value.toString() : value;

        return {
            hook(address, callback, resultType, paramsTypes, ...optionalArgs) {
                if (typeof callback !== "function") throw new TypeError("Hook callback must be a function");
                if (paramsTypes === undefined) {
                    paramsTypes = new Array(Math.max(0, callback.length - 1)).fill(module.exports.NativeType.Pointer);
                }

                const priority = optionalArgs[0] === undefined
                    ? module.exports.HookPriority.Normal
                    : optionalArgs[0];
                const suspendThreads = optionalArgs[1] === undefined ? true : optionalArgs[1];
                const pluginName = optionalArgs[2] === undefined
                    ? module.exports.getPluginName()
                    : optionalArgs[2];
                const asString = optionalArgs[3] === undefined ? false : optionalArgs[3];
                const callbackId = module.exports.callExportedFunction("nextHookCallbackId", false);
                const callbackName = `hook#${callbackId}`;

                ll.exports((originalAddress, params, hookId) => {
                    const result = callback(
                        (...originalParams) => {
                            if (originalParams.length !== paramsTypes.length) {
                                throw new Error("Wrong number of parameters passed to hook origin");
                            }
                            module.exports.callExportedFunction("markHookOriginCalled", false, hookId);
                            return module.exports.dynamicCall(
                                originalAddress,
                                resultType,
                                paramsTypes,
                                originalParams,
                    ...
                    return resultType === module.exports.NativeType.LongLong
                        || resultType === module.exports.NativeType.UnsignedLongLong
                        ? normalizeInteger64(result)
                        : result;
                }, pluginName, callbackName);
            ...
            dynamicCall(address, returnType, paramsType, params, asString = false) {
                return module.exports.callExportedFunction(
                    "dynamicCall",
                    false,
                    address,
                    returnType,
                    paramsType,
                    params.map(normalizeInteger64),
                    asString
                );
            },
            setLongLong(address, data, pauseThread = false) {
                return module.exports.callExportedFunction(
                    "setLongLong",
                    false,
                    address,
            ...
            setUnsignedLongLong(address, data, pauseThread = false) {
                return module.exports.callExportedFunction(
                    "setUnsignedLongLong",
                    false,
                    address,
    ...
    ...(() => [
        ...
        name => ({
            [name](data) {
                return module.exports.callExportedFunction(name, true, data?.getData?.("_pointer_") ?? data)?.value;
            }
        })
    )
    )(),
    ...(() => [
        ...
        name => ({
            [name](...args) {
                return module.exports.callExportedFunction(name, false, ...args);
            }
        })
    )
    )()
...
try {
    /**
     * @typedef {{ name: string, version?: string }} Dependency
     * @type {{
     *      entry: string,
     *      name: string,
     *      type: string
     ...
     * }}
     */
    const manifest = JSON.parse(File.readFrom(`./plugins/${module.exports.getPluginName()}/manifest.json`));
    if (
        !manifest.dependencies?.some(dependency => dependency.name === "iListenAttentively-LseExport")
        && !manifest.optionalDependencies?.some(dependency => dependency.name === "iListenAttentively-LseExport")
    ) {
        logger.warn(`The plugin "${manifest.name}" is missing the dependency "iListenAttentively-LseExport"`);
        (manifest.dependencies ??= []).push({ name: "iListenAttentively-LseExport" });
        File.writeTo(`./plugins/${module.exports.getPluginName()}/manifest.json`, JSON.stringify(manifest, null, 4));
        logger.warn(`The dependency "iListenAttentively-LseExport" has been added to the manifest.json file.`);
    }
} catch { }
